...
export default function PosPage() {
  ...
  const loadTaxRate = async () => {
    try {
      const { data, error } = await supabase.from("app_settings").select("value").eq("key", "tax_rate").single();
  ...
  const loadCustomers = async () => {
    const { data, error } = await supabase.from("customers").select("id, identification, name, phone, email").order("created_at", { ascending: false }).limit(200);
    if (error) throw new Error(error.message);
    const mapped: Customer[] = (data ?? []).filter((c: any) => c?.id).map((c: any) => ({
      id: String(c.id), identification: String(c.identification ?? ""), name: String(c.name ?? ""),
      phone: c.phone ? String(c.phone) : null, email: c.email ? String(c.email) : null,
    }));
    setCustomers(mapped);
    const cf = mapped.find((x) => x.identification === "CF") || mapped.find((x) => x.name?.toUpperCase() === "CONSUMIDOR FINAL");
    setSelectedCustomerId(cf ? cf.id : null);
  };

  const ensureConsumidorFinal = async (): Promise<Customer> => {
    const inState = customers.find((x) => x.identification === "CF") || customers.find((x) => x.name?.toUpperCase() === "CONSUMIDOR FINAL");
    if (inState) return inState;
    const { data: existing, error: exErr } = await supabase.from("customers").select("id,identification,name,phone,email").or("identification.eq.CF,name.ilike.%CONSUMIDOR FINAL%").limit(1).maybeSingle();
    if (exErr) throw new Error(exErr.message);
    if (existing?.id) return { id: String(existing.id), identification: String(existing.identification ?? "CF"), name: String(existing.name ?? "CONSUMIDOR FINAL"), phone: existing.phone ? String(existing.phone) : null, email: existing.email ? String(existing.email) : null };
    const { data: created, error: cErr } = await supabase.from("customers").insert({ identification: "CF", name: "CONSUMIDOR FINAL", phone: null, email: null }).select("id,identification,name,phone,email").single();
    if (cErr || !created) throw new Error(cErr?.message ?? "No se pudo crear CONSUMIDOR FINAL.");
    return { id: String(created.id), identification: String(created.identification ?? "CF"), name: String(created.name ?? "CONSUMIDOR FINAL"), phone: created.phone ? String(created.phone) : null, email: created.email ? String(created.email) : null };
  };
  ...
  const createCustomerQuick = async () => {
    ...
    setPayError(null); setCreatingCustomer(true);
    try {
      const { data, error } = await supabase.from("customers").insert({ identification, name, phone: newCustPhone.trim() || null, email: newCustEmail.trim() || null }).select("id,identification,name,phone,email").single();
      if (error || !data) throw new Error(error?.message ?? "Error creando cliente.");
      const created: Customer = { id: String(data.id), identification: String(data.identification ?? ""), name: String(data.name ?? ""), phone: data.phone ? String(data.phone) : null, email: data.email ? String(data.email) : null };
      setCustomers((prev) => [created, ...prev].slice(0, 200));
      setSelectedCustomerId(created.id);
      setNewCustId(""); setNewCustName(""); setNewCustPhone(""); setNewCustEmail("");
      setSaleOkMsg("Cliente creado ✅"); setShowCreateCustomer(false); setIsCustomerDropdownOpen(false);
    } catch (e: any) { setPayError(e?.message ?? "No se pudo crear el cliente."); }
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const { data: shift, error: shiftErr } = await supabase.from("shifts").select("id,status,opened_at").eq("branch_id", id).eq("status", "OPEN").order("opened_at", { ascending: false }).limit(1).maybeSingle();
      ...
      if (shiftDate !== today) { setStaleShift({ id: shift.id, opened_at: shift.opened_at }); setLoading(false); return; }
      setShiftId(shift.id);
      ...
      const { data: rows, error: prodErr } = await supabase
        ...
        .select("id, product_id, price, is_favorite, stock, products(name,image_url)")
        .eq("branch_id", id)
        .eq("is_active", true)
      ...
      const mapped: PosProduct[] = (rows ?? []).map((r: any) => ({
        branch_product_id: r.id, product_id: r.product_id,
        name: r.products?.name ?? "Producto", price: Number(r.price ?? 0),
        is_favorite: Boolean(r.is_favorite), image_url: r.products?.image_url ?? null,
        stock: Number(r.stock ?? 0), // ✅
      }));
      setProducts(mapped);
      setLoading(false);
    };
    run().catch((e: any) => { setPageError(e?.message ?? "Error cargando POS."); setLoading(false); });
  }, [router]);

  const closeStaleShift = async () => {
    if (!staleShift) return;
    setStaleErr(null); setClosingStale(true);
    try {
      const { data: totalsData } = await supabase.rpc("get_shift_totals", { p_shift_id: staleShift.id });
      const totals = Array.isArray(totalsData) ? totalsData[0] : totalsData;
      const expected = Number(totals?.expected_total ?? 0);
      const { error } = await supabase.from("shifts").update({ status: "CLOSED", closed_at: new Date().toISOString(), expected_total: expected, confirmed_total: null, closed_without_confirm: true }).eq("id", staleShift.id);
      if (error) throw new Error(error.message);
      setStaleShift(null); window.location.reload();
    } catch (e: any) { setStaleErr(e.message ?? "Error cerrando turno."); }
    finally { setClosingStale(false); }
  };
  ...
  const addToCart = (p: PosProduct) => {
    ...
    setCart((prev) => {
      ...
      if (idx === -1) return [...prev, { branch_product_id: p.branch_product_id, product_id: p.product_id, name: p.name, unit_price: p.price, qty: 1, image_url: p.image_url ?? null }];
      const copy = [...prev];
      if (copy[idx].qty >= p.stock) return copy;
  ...
  const incQty = (id: string) => {
    ...
    setCart((prev) => prev.map((it) => {
      if (it.branch_product_id !== id) return it;
      const prod = products.find((p) => p.branch_product_id === id);
      if (prod && it.qty >= prod.stock) return it; // ✅ tope de stock
      return { ...it, qty: it.qty + 1 };
    }));
  };

  const decQty = (id: string) => { setSaleOkMsg(null); setCart((prev) => prev.map((it) => it.branch_product_id === id ? { ...it, qty: it.qty - 1 } : it).filter((it) => it.qty > 0)); };
  const removeItem = (id: string) => { setSaleOkMsg(null); setCart((prev) => prev.filter((it) => it.branch_product_id !== id)); };
  const clearCart = () => { setSaleOkMsg(null); setCart([]); sessionStorage.removeItem(`cart_${branchId}`); };
  ...
  const openPayModal = async () => {
    setPayError(null); setSaleOkMsg(null); setCash("0"); setCard("0"); setTransfer("0"); setQr("0");
    setSelectedBill(null); setCustomBillValue(""); setCustomerSearch(""); setIsCustomerDropdownOpen(false); setShowCreateCustomer(false);
    try { const cf = await ensureConsumidorFinal(); setSelectedCustomerId(cf.id); await loadCustomers(); } catch { setSelectedCustomerId(null); }
    setShowPay(true);
  };

  const closePayModal = () => setShowPay(false);
  const formatShift = (iso: string | null) => { if (!iso) return "Turno activo"; const d = new Date(iso); return `Turno: ${d.toLocaleTimeString("es-CO", { hour: "2-digit", minute: "2-digit" })}`; };

  const filteredCustomers = useMemo(() => {
    const q = customerSearch.trim().toLowerCase();
    if (!q) return customers.slice(0, 30);
    return customers.filter((c) => { const a = (c.identification ?? "").toLowerCase(); const b = (c.name ?? "").toLowerCase(); const p = (c.phone ?? "").toLowerCase(); const e = (c.email ?? "").toLowerCase(); return a.includes(q) || b.includes(q) || p.includes(q) || e.includes(q); }).slice(0, 30);
  }, [customers, customerSearch]);

  const selectedCustomer = useMemo(() => { if (!selectedCustomerId) return null; return customers.find((c) => c.id === selectedCustomerId) ?? null; }, [customers, selectedCustomerId]);

  const loadShiftHistory = async () => {
    if (!shiftId) return;
    setHistoryError(null); setHistoryLoading(true);
    try {
      const { data: salesRows, error: salesErr } = await supabase.from("sales").select("id, receipt_number, total, created_at, customer_id, shift_id, branch_id, customers(name,identification)").eq("shift_id", shiftId).eq("branch_id", branchId).order("created_at", { ascending: false }).limit(80);
      if (salesErr) throw new Error(salesErr.message);
      const mappedSales: SaleHistoryRow[] = (salesRows ?? []).map((s: any) => ({ id: String(s.id), receipt_number: s.receipt_number ?? null, total: Number(s.total ?? 0), created_at: String(s.created_at), customers: s.customers ? { name: String(s.customers.name ?? ""), identification: String(s.customers.identification ?? "") } : null }));
      ...
      const { data: payRows, error: payErr } = await supabase.from("payments").select("sale_id, method, amount").in("sale_id", saleIds);
      if (payErr) throw new Error(payErr.message);
      const grouped: Record<string, PaymentRow[]> = {};
      (payRows ?? []).forEach((p: any) => { const sid = String(p.sale_id); if (!grouped[sid]) grouped[sid] = []; grouped[sid].push({ method: p.method as PaymentMethod, amount: Number(p.amount ?? 0) }); });
      setHistoryPaymentsBySale(grouped);
      const { data: verif } = await supabase.from("shift_verifications").select("verified_at, verified_by_name, opening_cash, sales_total, expected_total, sales_count").eq("shift_id", shiftId).maybeSingle();
      setShiftVerification(verif ? { verified_at: String(verif.verified_at), verified_by_name: String(verif.verified_by_name), opening_cash: Number(verif.opening_cash ?? 0), sales_total: Number(verif.sales_total ?? 0), expected_total: Number(verif.expected_total ?? 0), sales_count: Number(verif.sales_count ?? 0) } : null);
      setHistoryLoading(false);
    } catch (e: any) { setHistoryLoading(false); setHistoryError(e?.message ?? "No se pudo cargar el historial."); }
  };
  ...
  const saveVerification = async () => {
    ...
    setVerifyError(null); setVerifying(true);
    try {
      const { data, error: rpcErr } = await supabase.rpc("get_shift_totals", { p_shift_id: shiftId });
      if (rpcErr) throw new Error(rpcErr.message);
      const row = Array.isArray(data) ? data[0] : data;
      const { data: sh } = await supabase.from("shifts").select("opening_cash").eq("id", shiftId).single();
      ...
      const { error: insErr } = await supabase.from("shift_verifications").insert({ shift_id: shiftId, branch_id: branchId, verified_by_name: name, opening_cash, sales_total, expected_total, sales_count });
      if (insErr) throw new Error(insErr.message);
      setShiftVerification({ verified_at: new Date().toISOString(), verified_by_name: name, opening_cash, sales_total, expected_total, sales_count });
      setShowVerifyForm(false); setVerifyName("");
    } catch (e: any) { setVerifyError(e.message ?? "No se pudo guardar la verificación."); }
    finally { setVerifying(false); }
  };

  const methodLabel = (m: string) => { if (m === "CASH") return "EFECTIVO"; if (m === "CARD") return "TARJETA"; if (m === "TRANSFER") return "TRANSFER"; if (m === "QR") return "QR"; return m; };

  const saveSale = async () => {
    if (!branchId || !shiftId) return;
    setPayError(null); setSavingSale(true);
    if (cart.length === 0) { setSavingSale(false); setPayError("El carrito está vacío."); return; }
    const sum = paymentsSum;
    if (Math.round((sum - total) * 100) / 100 !== 0) { setSavingSale(false); setPayError(`Los pagos no cuadran. Pagos: ${sum} / Total: ${total}`); return; }
    const payRows: { method: PaymentMethod; amount: number }[] = [];
    const c = toNum(cash); const ca = toNum(card); const tr = toNum(transfer); const q = toNum(qr);
    ...
    try { if (!customerId) { const cf = await ensureConsumidorFinal(); customerId = cf.id; } } catch { customerId = selectedCustomerId; }

    const { data: saleRow, error: saleErr } = await supabase.from("sales").insert({ branch_id: branchId, shift_id: shiftId, customer_id: customerId, subtotal, tax_total: taxTotal, total }).select("id,total").single();
    ...
    const itemsToInsert = cart.map((it) => ({ sale_id: saleId, product_id: it.product_id, qty: it.qty, unit_price: it.unit_price, line_total: Math.round(it.unit_price * it.qty * 100) / 100 }));
    ...
    const cashPaid = c;
    const { data: shiftRow, error: shGetErr } = await supabase.from("shifts").select("expected_cash").eq("id", shiftId).single();
    ...
    // ✅ Descontar stock atómicamente via RPC
    const stockItems = cart.map((it) => ({
      branch_id: branchId,
      branch_product_id: it.branch_product_id,
      product_id: it.product_id,
      qty: it.qty,
      name: it.name,
    }));
    const { error: stockErr } = await supabase.rpc("descontar_stock_venta", { p_items: stockItems });
    if (stockErr) { setSavingSale(false); setPayError(stockErr.message); return; }

    // ✅ Actualizar stock en estado local sin recargar
    setProducts((prev) => prev.map((p) => {
      const item = cart.find((c) => c.branch_product_id === p.branch_product_id);
      if (!item) return p;
      return { ...p, stock: p.stock - item.qty };
    }));

    setSavingSale(false); setShowPay(false); setCart([]);
    setSelectedBill(null); setCustomBillValue("");
      ...
      <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl">
        ...
        <div className="card-b space-y-4">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 space-y-1">
            ...
            <div className="text-sm text-amber-700">{new Date(staleShift.opened_at).toLocaleString("es-CO", { weekday: "long", day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
          </div>
          <div className="text-sm text-gray-600">Este turno <span className="font-extrabold text-gray-900">no fue cerrado</span> al final del día. Al cerrarlo ahora quedará marcado como <span className="font-extrabold text-amber-700">"cerrado sin confirmar"</span> para que el administrador lo revise.</div>
          {staleErr && <div className="alert-err">{staleErr}</div>}
          <button className="btn btn-primary w-full" onClick={closeStaleShift} disabled={closingStale}>{closingStale ? "Cerrando turno..." : "Cerrar turno pendiente y continuar"}</button>
        </div>
      </div>
    </div>
  );
        ...
        subtitle={
          <div className="mt-4 rounded-2xl border border-gray-200 bg-white/70 px-4 py-3 shadow-sm">
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <span className="inline-flex items-center rounded-full border border-gray-200 bg-gray-50 px-3 py-1 font-semibold text-gray-700">Sucursal: <span className="ml-1 font-extrabold">{branchName}</span></span>
              <span className="text-gray-300">•</span>
              <span className="inline-flex items-center rounded-full border border-blue-200 bg-blue-50 px-3 py-1 font-semibold text-blue-700">Turno: <span className="ml-1 font-extrabold">{formatShift(shiftOpenedAt)}</span></span>
              <span className="text-gray-300">•</span>
              <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 font-semibold text-emerald-700">Impuesto: <span className="ml-1 font-extrabold">{(taxRate * 100).toFixed(2)}%</span></span>
            </div>
          </div>
        }
      >
        <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
          ...
          <div className={savingSale ? "opacity-60 pointer-events-none" : ""}>
            ...
            <ProductsGrid products={favorite} onClick={addToCart} disabled={savingSale} cartQty={(id) => cart.find((c) => c.branch_product_id === id)?.qty ?? 0} />
            <div className="h-5" />
            <Section title="Todos" />
            <ProductsGrid products={others} onClick={addToCart} disabled={savingSale} cartQty={(id) => cart.find((c) => c.branch_product_id === id)?.qty ?? 0} />
          </div>
          ...
          <div className="card h-fit">
            <div className="card-h flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {/* ✅ Botón inventario */}
  <button className="btn" onClick={() => router.push("/inventario")} disabled={savingSale}>
    Inventario
  </button>
                <button className="btn" onClick={openHistoryModal} disabled={!shiftId || savingSale}>Historial</button>
                {shiftId && (<button className="btn" onClick={() => { if (cart.length > 0) { alert("No puedes cerrar turno con productos en el carrito."); return; } router.push("/close-shift"); }} disabled={savingSale}>Cerrar turno</button>)}
              </div>
            </div>
            ...
            <div className="card-b">
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-3">
                  {cart.map((it) => {
                    const prod = products.find((p) => p.branch_product_id === it.branch_product_id);
                    const maxQty = prod?.stock ?? it.qty;
                    return (
                      <div key={it.branch_product_id} className="rounded-2xl border border-gray-200 p-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="h-12 w-12 rounded-2xl border border-gray-200 bg-gray-50 overflow-hidden flex items-center justify-center">
                              {it.image_url ? <img src={it.image_url} alt={it.name} className="w-full h-full object-contain" /> : <div className="text-[10px] text-gray-400 font-bold">IMG</div>}
                            </div>
                            <div className="min-w-0">
                              ...
                              {prod && prod.stock <= 5 && (<div className="text-xs text-amber-600 font-semibold">Stock: {prod.stock} und.</div>)}
                            </div>
                          </div>
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <button className="btn px-3 py-1" disabled={savingSale || it.qty >= maxQty} onClick={() => incQty(it.branch_product_id)}>+</button>
          ...
          {showHistory && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
              <div className="card w-full max-w-2xl max-h-[85vh] flex flex-col">
                <div className="card-h flex items-center justify-between">
                  <div><div className="text-lg font-extrabold">Historial del turno</div><div className="text-sm text-gray-500">Ventas registradas en esta sucursal</div></div>
                  <div className="flex gap-2">
                    <button className="btn" onClick={loadShiftHistory} disabled={historyLoading}>{historyLoading ? "Cargando..." : "Refrescar"}</button>
                ...
                <div className="card-b flex-1 overflow-auto space-y-3">
                  ...
                  <div className="rounded-2xl border border-gray-200 p-3 bg-white space-y-2">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="text-sm"><span className="font-extrabold">Total vendido:</span> ${historySales.reduce((acc, s) => acc + Number(s.total ?? 0), 0).toLocaleString("es-CO")}</div>
                    </div>
                    {(() => {
                      const metodosMap: Record<string, { label: string; emoji: string; color: string }> = { CASH: { label: "Efectivo", emoji: "💵", color: "text-emerald-700" }, CARD: { label: "Tarjeta", emoji: "💳", color: "text-blue-700" }, TRANSFER: { label: "Transferencia", emoji: "🏦", color: "text-violet-700" }, QR: { label: "QR", emoji: "📲", color: "text-orange-700" } };
                      const totalesPorMetodo = Object.values(historyPaymentsBySale).flat().reduce((acc, p) => { const metodo = String(p.method ?? "CASH").toUpperCase(); acc[metodo] = (acc[metodo] ?? 0) + Number(p.amount ?? 0); return acc; }, {} as Record<string, number>);
                      const metodosUsados = Object.entries(totalesPorMetodo).filter(([, v]) => v > 0);
                      if (metodosUsados.length === 0) return null;
                      return (<div className="flex flex-wrap gap-2 border-t border-gray-100 pt-2">{metodosUsados.map(([metodo, monto]) => { const info = metodosMap[metodo] ?? { label: metodo, emoji: "💰", color: "text-gray-700" }; return (<div key={metodo} className="flex items-center gap-1 rounded-2xl border border-gray-200 bg-gray-50 px-3 py-1"><span className="text-sm">{info.emoji}</span><span className={`text-xs font-bold ${info.color}`}>{info.label}:</span><span className={`text-xs font-extrabold ${info.color}`}>${monto.toLocaleString("es-CO")}</span></div>); })}</div>);
                    })()}
                    {historyOpeningCash !== null && (<div className="flex items-center justify-between border-t border-gray-100 pt-2"><span className="text-sm text-gray-500">Base al abrir turno</span><span className="text-sm font-extrabold text-gray-800">${historyOpeningCash.toLocaleString("es-CO")}</span></div>)}
                    <div className="border-t border-gray-100 pt-2">
                      {shiftVerification ? (
                        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 space-y-1">
                          ...
                          <div className="text-xs text-emerald-700"><span className="font-bold">Cajero:</span> {shiftVerification.verified_by_name}</div>
                          <div className="text-xs text-emerald-700"><span className="font-bold">Hora:</span> {new Date(shiftVerification.verified_at).toLocaleString("es-CO")}</div>
                          <div className="text-xs text-emerald-700"><span className="font-bold">Base:</span> ${shiftVerification.opening_cash.toLocaleString("es-CO")} • <span className="font-bold">Ventas:</span> ${shiftVerification.sales_total.toLocaleString("es-CO")} • <span className="font-bold">Total:</span> ${shiftVerification.expected_total.toLocaleString("es-CO")}</div>
                          <div className="text-xs text-emerald-600 opacity-75">El cajero confirmó que los valores eran correctos al momento de la verificación.</div>
                        </div>
                      ) : showVerifyForm ? (
                        <div className="rounded-2xl border border-gray-200 p-3 space-y-3">
                          ...
                          <div className="text-xs text-gray-500">Al verificar confirmas que los valores actuales son correctos y aceptas responsabilidad desde este momento.</div>
                          <label className="grid gap-1"><span className="label">Tu nombre</span><input className="input" value={verifyName} onChange={(e) => setVerifyName(e.target.value)} placeholder="Ej: Juan Pérez" disabled={verifying} /></label>
                          <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3 space-y-1">
                            ...
                            <div className="flex justify-between text-xs"><span className="text-gray-500">Base de caja</span><span className="font-bold">${(historyOpeningCash ?? 0).toLocaleString("es-CO")}</span></div>
                            <div className="flex justify-between text-xs"><span className="text-gray-500">Total ventas</span><span className="font-bold">${historySales.reduce((acc, s) => acc + Number(s.total ?? 0), 0).toLocaleString("es-CO")}</span></div>
                            <div className="flex justify-between text-xs border-t border-gray-200 pt-1 mt-1"><span className="text-gray-700 font-bold">Total esperado en caja</span><span className="font-extrabold">${((historyOpeningCash ?? 0) + historySales.reduce((acc, s) => acc + Number(s.total ?? 0), 0)).toLocaleString("es-CO")}</span></div>
                          </div>
                          {verifyError && <div className="alert-err">{verifyError}</div>}
                          <div className="flex gap-2">
                            <button className="btn flex-1" onClick={() => { setShowVerifyForm(false); setVerifyName(""); setVerifyError(null); }} disabled={verifying}>Cancelar</button>
                            <button className="btn btn-primary flex-1" onClick={saveVerification} disabled={verifying}>{verifying ? "Guardando..." : "Confirmar verificación"}</button>
                          </div>
                        </div>
                      ) : (
                        <button className="btn w-full" onClick={() => { setShowVerifyForm(true); setVerifyError(null); }}>Verificar entrega de caja</button>
                      )}
                    </div>
                  </div>
                  {historyLoading ? <div className="text-sm text-gray-500">Cargando historial…</div> : historySales.length === 0 ? <div className="text-sm text-gray-500">Aún no hay ventas en este turno.</div> : (
                    <div className="space-y-2">
                      {historySales.map((s) => {
                        const receipt = s.receipt_number ? `LF-${String(s.receipt_number).padStart(6, "0")}` : String(s.id).slice(0, 8).toUpperCase();
                        ...
                        const custText = s.customers?.name ? `${s.customers.name}${s.customers.identification ? ` (${s.customers.identification})` : ""}` : "CONSUMIDOR FINAL";
                        return (<div key={s.id} className="rounded-2xl border border-gray-200 p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-sm font-extrabold truncate">{time} • {receipt}</div><div className="text-xs text-gray-500 truncate">{custText}</div><div className="mt-1 text-xs text-gray-600">{payText}</div></div><div className="text-sm font-extrabold whitespace-nowrap">${Number(s.total).toLocaleString("es-CO")}</div></div></div>);
                      })}
                    </div>
                  )}
                </div>
          ...
          {printingSaleId && (<PrintPortal><div className="print-layer"><TicketInline saleId={printingSaleId} onPrinted={() => setPrintingSaleId(null)} /></div></PrintPortal>)}
        </div>
      </PageShell>
    </div>
  );
...
function Section({ title }: { title: string }) {
  return (<div className="flex items-center justify-between mb-3"><h2 className="text-lg font-extrabold">{title}</h2><span className="badge">Tap para agregar</span></div>);
}

// ✅ ProductsGrid con soporte de stock
function ProductsGrid({ products, onClick, disabled, cartQty }: {
  products: PosProduct[];
  ...
  return (
    <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 xl:grid-cols-4">
      {products.map((p) => {
        ...
        const bloqueado = disabled || sinStock;
        return (
          <button key={p.branch_product_id} disabled={bloqueado} onClick={() => onClick(p)}
            className={`card p-3 text-left transition ${bloqueado ? "opacity-50 cursor-not-allowed" : "hover:shadow-md active:scale-[0.99]"}`}>
            <div className="w-full aspect-square rounded-2xl border border-gray-200 bg-gray-50 overflow-hidden flex items-center justify-center">
              {p.image_url ? <img src={p.image_url} alt={p.name} className="w-full h-full object-contain" loading="lazy" /> : <div className="text-xs text-gray-400 font-semibold">Sin imagen</div>}
            </div>
            <div className="mt-3">
              ...
              {/* ✅ Siempre muestra stock */}
              {sinStock ? (
                <div className="mt-1 text-xs font-extrabold text-red-600">Sin stock</div>
              ) : p.stock <= 5 ? (
                <div className="mt-1 text-xs font-extrabold text-amber-600">{p.stock} disponibles</div>
              ) : (
                <div className="mt-1 text-xs font-semibold text-gray-400">{p.stock} disponibles</div>
              )}
            </div>
          </button>
        );
...
function TicketInline({ saleId, onPrinted }: { saleId: string; onPrinted: () => void }) {
  ...
  const methodLabel = (m: string) => { if (m === "CASH") return "EFECTIVO"; if (m === "CARD") return "TARJETA"; if (m === "TRANSFER") return "TRANSFERENCIA"; if (m === "QR") return "QR"; return m; };
  useEffect(() => {
    const run = async () => {
      const { data: saleRow } = await supabase.from("sales").select("id, receipt_number, subtotal, tax_total, total, created_at, branch_id, branches(name)").eq("id", saleId).single();
      const { data: itemsRows } = await supabase.from("sale_items").select("qty, unit_price, line_total, products(name)").eq("sale_id", saleId);
      const { data: payRows } = await supabase.from("payments").select("method, amount").eq("sale_id", saleId);
      setSale(saleRow); setItems(itemsRows ?? []); setPayments(payRows ?? []);
  ...
  const receipt = sale.receipt_number ? `LF-${String(sale.receipt_number).padStart(6, "0")}` : String(sale.id).slice(0, 8).toUpperCase();
  const branchName = sale.branches?.name ?? "Sucursal";
  return (
    <div className="ticket">
      <div className="center"><img src="/logo.png" alt="logo" style={{ width: "120px", marginBottom: "8px" }} /></div>
      ...
      <div className="line" />
      {items.map((it, i) => (<div key={i} className="item"><div className="row"><div className="left">{it.qty} x {it.products?.name ?? "Producto"}</div><div className="right">${Number(it.line_total).toLocaleString("es-CO")}</div></div><div className="muted">Unit: ${Number(it.unit_price).toLocaleString("es-CO")}</div></div>))}
      <div className="line" />
      ...
      {payments.map((p, i) => (<div key={i} className="row"><div className="left">{methodLabel(p.method)}</div><div className="right">${Number(p.amount).toLocaleString("es-CO")}</div></div>))}
